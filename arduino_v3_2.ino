...
void setup() {
    ...
    // Attente reseau (timeout 60s, 1 retry)
    int16_t sq;
    RegStatus status = REG_NO_RESULT;
    for (int netAttempt = 0; netAttempt < 2; netAttempt++) {
        Serial.printf("Tentative reseau %d/2\n", netAttempt + 1);
        unsigned long netStart = millis();
        status = REG_NO_RESULT;
        while (status == REG_NO_RESULT || status == REG_SEARCHING || status == REG_UNREGISTERED) {
            if (millis() - netStart > 60000) {
                Serial.printf("Reseau timeout 60s (tentative %d)\n", netAttempt + 1);
                break;
            }
        ...
        if (status == REG_OK_HOME || status == REG_OK_ROAMING) break;
        if (netAttempt == 0) {
            Serial.println("Retry reseau - reset modem...");
            modem.sendAT(GF("+CFUN=1,1"));
            modem.waitResponse(5000L);
            delay(3000);
            int atRetry = 0;
            while (!modem.testAT(1000) && atRetry++ < 5) delay(1000);
        }
    }
    if (status != REG_OK_HOME && status != REG_OK_ROAMING) {
        Serial.println("Reseau inaccessible apres 2 tentatives - retour sommeil");
        goToSleep();
    }
    Serial.println();
...
void readAndSendData() {
    ...
    // Log detaille pluie pour debug
    Serial.printf("[PLUIE] rainCount=%d lastSent=%d wokeFromRain=%s totalRainfall=%.3f cumulative=%d\n", 
        rainCount, lastSentRainCount, wokeFromRain ? "true" : "false", totalRainfall, cumulativeRainCount);
...
void loop() {
    ...
    if (wokeFromRain) {
        Serial.println("[MODE_PLUIE] Actif - 6 minutes max");
        ...
        while (millis() - rainStart < RAIN_AWAKE_MS) {
            if (rainCount > lastSentRainCount && extensions < RAIN_MAX_EXTENSIONS) {
                Serial.printf("[MODE_PLUIE] Extension %d/%d - nouvelle bascule detectee\n", extensions + 1, RAIN_MAX_EXTENSIONS);
                delay(5000);
                readAndSendData();
                extensions++;
            }
        ...
        Serial.printf("[MODE_PLUIE] Fin - %d extensions utilisees\n", extensions);
        wokeFromRain = false;
    }
